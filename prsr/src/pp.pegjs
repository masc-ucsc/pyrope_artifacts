{
 function buildBinaryExpression(head, tail) {
   return tail.reduce(function(result, element) {
     return {
       type: "binary_expression",
       operator: element[1],
       left: result,
       right: element[3]
     };
   }, head);
 }

 function buildMultiLineBinaryExpression(head, tail) {
   return tail.reduce(function(result, element) {
     return {
       type: "binary_expression",
       operator: element[1],
       left: result,
       right: element[2]
     };
   }, head);
 }

 function extractList(list, index) {
   return list.map(function(element) { return element[index]; });
 }

 function buildList(head, tail, index) {
   return [head].concat(extractList(tail, index));
 }

 function optionalList(value) {
   return value !== null ? value : [];
 }

 function extractOptional(optional, index) {
   return optional ? optional[index] : null;
 }
 
 function prettyPrint(head,tail){
 	var i,tmp=[]
    if((head!=null)){
    	tail.unshift(head)
    }
    return tail
 }
 
 function prettyPrintScope(head,tail,end){
 	var i,tmp=[]
    if(!(tail instanceof Array) && head==null){
    	tmp.push(tail)
        return tmp
    }
    if(head==null){
    	return head
    }
    
    if(!(tail instanceof Array) && tail!=null){
    	head.push(tail)
    }
    
    if(end){
    	for(i=0;i<end.length;i++){ 
        	head.push(end[i])
    	}
    }
   	return head
 }
 
 function prettyPrintArray(head,tail){
 	var tmp
    tmp = tail.reduce(function flat(a,b) {  	
        if(Array.isArray(b)){
        	return b.reduce(flat,a)
        }
        if(b instanceof Object){
        	b = b.value
        }
     	a.push(b)
        return a       
   	},[]);
    tmp.unshift(head.value)
    return tmp.join('')
 }
}


start
  	= head:(code_blocks? __) {
    	var i,j,tmp=[]
        if(head[0]==null){
        	head.splice(0,1)
            return head[0]
        }
        
    	for(i=0;i<head[1].length;i++){ 
        	head[0].push(head[1][i])
    	}
        return head[0]
    }

code_blocks
  = head:code_block_int tail:(y:EOS x:code_block_int{return prettyPrint(y,x)})* {
   	tail.unshift(head)
    var i,j,tmp=[]
    for(i=0;i<tail.length;i++){ 
    	for(j=0;j<tail[i].length;j++){
       		tmp.push(tail[i][j])
        }
    }
    return tmp
  	//return tail
  }

code_block_int
        = x:__ head:(if_statement/for_statement/while_statement/assignment_expression/function_pipe/fcall_implicit/fcall_explicit
        /return_statement/negation_statement/tail:assertion_statement {return tail})   {
        	x.push(head)
            return x
        	//return head
        }
    
scope_declaration
	= args:scope LBRACE sc:((EOS/__) __) body:(x:scope_body? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)})
   	RBRACE alt:(__ x:scope_else{return x})? 
    {
    	var i,j
        for(i=0;i<sc[1].length;i++){
         	sc.push(sc[1][i])
       	}
       	sc.splice(1,1)
        if(sc[0]==null || (sc[0] instanceof Array)){
        	sc.splice(0,1)
        }
        //return body
        if(body==null){
        	body=[]
        }
        j=sc.length-1
        while(sc.length>0 && j>=0){
        	body.unshift(sc[j])
            j = j-1
        }
        if((body instanceof Array) && body.length==0) body=null
   
  		return {
     		type:"function_declaration",
       		args:args,
       		true_case:body,
       		false_case:alt
        }
   	}

scope_else
  	= ELSE _ LBRACE sc:((EOS/__) __) else_true:(x:scope_body? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE
  	{
    	var i,j
        for(i=0;i<sc[1].length;i++){
         	sc.push(sc[1][i])
       	}
       	sc.splice(1,1)
        if(sc[0]==null || (sc[0] instanceof Array)){
        	sc.splice(0,1)
        }
		//return sc
        if(else_true==null){
        	else_true=[]
        }
        j=sc.length-1
        while(sc.length>0 && j>=0){
        	else_true.unshift(sc[j])
            j = j-1
        }
        if((else_true instanceof Array) && else_true.length==0)else_true=null
   		return else_true;
   	}

scope_body
	= head:code_blocks tail:(EOS x:logical_expression __ {return x})? {
    	if(tail){
          head.push(tail)
        }
        return head
    }
    / head:logical_expression __ {return head}
    
scope
        = COLON args:scope_condition? COLON {
        return args
   }

scope_condition
        =  head:scope_argument tail:(WHEN x:(logical_expression) {return x} )? {
     return {
       arg_list:head,
       condition:tail
     }
   }
   / head:(x:scope_argument {return x})? WHEN tail:(logical_expression)  {
     return {
       arg_list:head,
       condition:tail
     }
   }

scope_argument
        = LPAR head:identifier tail:(_ identifier)* RPAR {
        var char = buildList(head, tail, 1);
       return char
   }
   / LPAR head:identifier tail:(COMMA identifier)* RPAR {
        var char = buildList(head, tail, 1);
       return char
   }

if_statement
        = IF if_test:logical_expression sc:((_ x:"::" LBRACE y:(EOS/__) )/_ LBRACE x:(EOS/__) {return x})  
        if_true:(x:code_blocks? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE ELSE:(__ x:else_statement{return x})? 
   	{
        if((sc instanceof Array))sc=null
        if(if_true!=null){
        	if(sc && sc.length!=4){
        		if_true.unshift(sc)
            	sc=null
        	} else if(sc && sc.length==4){
        		if_true.unshift(sc[3])
            	sc=sc[1]
        	}
        } else if(if_true==null) {
        	if(sc && sc.length!=4){
            	if_true=[sc]
                sc=null
        	} else if(sc && sc.length==4){
            	if_true=[sc[3]]
                sc=sc[1]                
        	}
        }
        //return if_true
        //return sc        
		return {
       		start_pos:location().start.offset,
        	end_pos:location().end.offset,
            type:"if_statement",
            condition:if_test,
       		scope:sc,
       		true_case:if_true,
       		false_case:ELSE
     	}
   	}
        
else_statement
        = ELIF else_test:logical_expression  sc:((_ x:"::" LBRACE (EOS/__))/_ LBRACE x:(EOS/__) {return x}) 
        else_true:(x:code_blocks? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE else_false:(__ x:else_statement{return x})? 
  	{
    	if((sc instanceof Array))sc=null
   		if(else_true!=null){
        	if(sc && sc.length!=4){
        		else_true.unshift(sc)
            	sc=null
        	} else if(sc && sc.length==4){
        		else_true.unshift(sc[3])
            	sc=sc[1]
        	}
        } else if(else_true==null) {
        	if(sc && sc.length!=4){
            	else_true=[sc]
                sc=null
        	} else if(sc && sc.length==4){
            	else_true=[sc[3]]
                sc=sc[1]                
        	}
        }
     	return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
       		type:"if_statement",
            condition:else_test,
       		scope:sc,
       		true_case:else_true,
       		false_case:else_false
     	};
   	}
   / ELSE  sc:((_ x:"::" LBRACE (EOS/__))/_ LBRACE x:(EOS/__) {return x}) 
   else_true:(x:code_blocks? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE    //else
  	{
    	if((sc instanceof Array))sc=null
   		if(else_true!=null){
        	if(sc && sc.length!=4){
        		else_true.unshift(sc)
            	sc=null
        	} else if(sc && sc.length==4){
        		else_true.unshift(sc[3])
            	sc=sc[1]
        	}
        } else if(else_true==null) {
        	if(sc && sc.length!=4){
            	else_true=[sc]
                sc=null
        	} else if(sc && sc.length==4){
            	else_true=[sc[3]]
                sc=sc[1]                
        	}
        }
        
     	return else_true;
   	}

for_statement
  	= FOR idx:for_index sc:((_ x:"::" LBRACE (EOS/__))/_ LBRACE x:(EOS/__) {return x}) 
    body:(x:code_blocks? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE 
    {
    	if((sc instanceof Array))sc=null
    	if(body!=null){
        	if(sc && sc.length!=4){
        		body.unshift(sc)
            	sc=null
        	} else if(sc && sc.length==4){
        		body.unshift(sc[3])
            	sc=sc[1]
        	}
        } else if(body==null) {
        	if(sc && sc.length!=4){
            	body=[sc]
                sc=null
        	} else if(sc && sc.length==4){
            	body=[sc[3]]
                sc=sc[1]                
        	}
        }
        
    	return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
        	type:"for_statement",
            index:idx,
           	scope:sc,
           	body:body
       	}
   	}

for_index
        = head:rhs_expression_property tail:(_ rhs_expression_property)* {
        //return head
       var char = buildList(head, tail, 1);
       return char
   }
   / head:rhs_expression_property tail:(COMMA rhs_expression_property)* {
        //return head
       var char = buildList(head, tail, 1);
       return char
   }

while_statement
   = WHILE cond:logical_expression sc:((_ x:"::" LBRACE (EOS/__))/_ LBRACE x:(EOS/__) {return x}) 
   	body:(x:code_blocks? y:(EOS/__) z:__ {return prettyPrintScope(x,y,z)}) RBRACE 
    {
    	if((sc instanceof Array))sc=null
    	if(body!=null){
        	if(sc && sc.length!=4){
        		body.unshift(sc)
            	sc=null
        	} else if(sc && sc.length==4){
        		body.unshift(sc[3])
            	sc=sc[1]
        	}
        } else if(body==null) {
        	if(sc && sc.length!=4){
            	body=[sc]
                sc=null
        	} else if(sc && sc.length==4){
            	body=[sc[3]]
                sc=sc[1]                
        	}
        }
        
 		return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
     		type:"while_statement",
            condition:cond,
            scope:sc,
           	body:body
    	}
   	}

return_statement
	= RETURN head:logical_expression {
    	return{
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
       		type:"return_statement",
            argument:head
     	}
    }
    
assertion_statement
  	= ASSERTION head:logical_expression {
 		return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
   			type:"assertion_statement",
       		condition:head
  		}
	}

negation_statement
   	= NEGATION head:(logical_expression/assertion_statement) EOS {
    	return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
       		type:"negation_statement",
            condition:head
     	}
   	}

compile_check_statement
   	= COMPILE_CHECK (EOS/__) head:code_blocks* __ END {
     	return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
       		type:"compile_check_statement",
            body:head
     	}
   	}

assignment_expression
        =  head:(overload_notation/lhs_expression) __ op:assignment_operator __
   tail:(function_pipe/fcall_implicit/rhs_expression_property/logical_expression) prop:(_ x:rhs_expression_property{return x})*  {
  	if(prop.length!=0) {
       	prop.unshift(tail)
      	return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
            type:"assignment_expression",
            operator:op,
            left:head,
            property:prop
        }
  	}
    return {
    	start_pos:location().start.offset,
       	end_pos:location().end.offset,    
        type:"assignment_expression",
        operator:op,
        left:head,
        right:tail
   	}
  }     

fcall_implicit
        = !constant func:tuple_dot_notation !not_in_implicit head:(white_space+ x:(rhs_expression_property/tuple_notation){return x})?
   tail:(_ (rhs_expression_property/tuple_notation))* _ scope:scope_declaration? {
        var char = buildList(head, tail, 1);
        return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
         	type:"function_call",
         	function:func,
         	arguments:char,
         	body:scope
       }
     }
     // fcall_dot_notation
     /* !constant func:tuple_dot_notation _ scope:scope_declaration {
       return {
         type:"function_call",
         function:func,
         //arguments:null,
         body:scope
       }
     }*/

not_in_implicit
 = (_ assignment_operator)
   / (_ ".(")
   / (":")
   / ".."
   / "["
   / "[["
   / __ PLUS
   / __ MINUS
   / __ overload_notation
   / __ STAR
   / __ DIV
   / __ T_PLUS
   / __ T_STAR
   / __ OR
   / __ AND
   / __ EQUEQU
   / __ ISEQU
   / __ BANGEQU
   / __ LE
   / __ GE
   / __ LT
   / __ GT
   / __ UNION
   / __ INTERSECT
   / __ IN
   / __ HAT
   / __ AMPERSAND
   
fcall_implicit_no_arg
        = !constant func:tuple_dot_notation _ {
       	return {
         	type:"function_call",
         	function:func,
         	arguments:null,
         	body:null
    	}
   	}

fcall_explicit
        = !constant func:tuple_dot_notation DOT arg:fcall_arg_notation scope:scope_declaration? {
        return {
        	start_pos:location().start.offset,
        	end_pos:location().end.offset,
        	type:"function_call",
           	function:func,
           	arguments:arg,
           	body:scope
       }
   }

fcall_arg_notation
        = LPAR head:(rhs_expression_property/lhs_var_name) tail:(_ (rhs_expression_property/lhs_var_name))* RPAR {
        var char = buildList(head, tail, 1);
       return char
   }
   / LPAR head:logical_expression tail:(COMMA logical_expression)* RPAR {
        var char = buildList(head, tail, 1);
       return char
   }
   / LPAR RPAR {return null}

function_pipe
	= head:(fcall_implicit/logical_expression) _ tail:(FUNC_PIPE x:(fcall_implicit/fcall_explicit) _ {return x})+ {
  		return tail.reduce(function(result, element) {
       		return {
          		type: "function_call",
                start_pos:location().start.offset,
         		end_pos:location().end.offset,
                arguments: result,
                function: element,
      		};
    	}, head);
   	}

/*
logical_expression_or
 =  OR _ relational_expression ((_ (logical_expression_or/logical_expression_and))/(__ logical_expression_or))*

logical_expression_and
 =  AND _ relational_expression ((_ (logical_expression_or/logical_expression_and))/(__ logical_expression_and))*

logical_expression
        = head:relational_expression tail:(_ (logical_expression_or/logical_expression_and))*   
        / head:relational_expression tail:(__ (logical_expression_or/logical_expression_and))* 
*/


logical_expression
        =  head:relational_expression tail:(__ (OR/AND) _ relational_expression)* {
     return buildBinaryExpression(head, tail);
   }

relational_expression
        =  head:additive_expression tail:(__ (EQUEQU/ ISEQU/ BANGEQU/ LE/ GE/ LT/ GT) _ additive_expression)* {
     return buildBinaryExpression(head, tail);
   }

additive_expression
        = head:bitwise_expression tail:(__ (T_PLUS/PLUS/MINUS/LEFT/RIGHT/T_STAR/UNION/INTERSECT/IN/overload_notation) _ bitwise_expression)* {
     return buildBinaryExpression(head, tail);
   }

bitwise_expression
        = head:multiplicative_expression tail:(__ (HAT/AMPERSAND) _ multiplicative_expression)* {
     return buildBinaryExpression(head, tail);
   }

multiplicative_expression
        = head:factor tail:(__ (STAR/DIV) _ factor)* {
     return buildBinaryExpression(head, tail);
   }

factor
        = LPAR __ head:logical_expression __ RPAR {return head;}
   / rhs_expression

tuple_by_notation
        = white_space+ BY head:lhs_var_name {return head}

tuple_notation_no_bracket
   = head:bit_selection_notation tail:(_ bit_selection_notation)* {
       	var char = buildList(head, tail, 1);
        return {
        	type:"tuple_list",
        	elements:char,
        }
   }
   //a[1 2] works; a[1+2,3] not working
   /* head:(rhs_expression_property/logical_expression/range_notation/bit_selection_notation) _
   tail:(COMMA (rhs_expression_property/logical_expression/range_notation/bit_selection_notation) __)*
   {
       var char = buildList(head, tail, 1);
       return {
        	type:"tuple_list",
        	elements:char,
       }
   }*/
   
tuple_notation
  	= LPAR head:(rhs_expression_property/logical_expression/range_notation/bit_selection_notation/tuple_notation) _
   	tail:(COMMA (rhs_expression_property/logical_expression/range_notation/bit_selection_notation/tuple_notation) __)* RPAR 
   	end:(LBRK LBRK prop:(logical_expression/tuple_notation_no_bracket)? RBRK RBRK{return prop}) {
    	var char = buildList(head, tail, 1);
        return {
        	type:"tuple_list",
        	elements:char,
            bit_sel:end
       }
    }
	
    / LPAR head:bit_selection_notation tail:(_ bit_selection_notation)* RPAR 
	end:(LBRK LBRK prop:(logical_expression/tuple_notation_no_bracket)? RBRK RBRK{return prop}) {
    	var char = buildList(head, tail, 1);
        return {
        	type:"tuple_list",
        	elements:char,
            bit_sel:end
       }
    }
  	
    / LPAR head:bit_selection_notation tail:(_ bit_selection_notation)* RPAR by:tuple_by_notation? {
       if (by) {
       		var char = buildList(head, tail, 1);
            return {
            	type:"tuple_list",
            	elements:char,
            	skip_by:by
        	}
     	}
       	var char = buildList(head, tail, 1);
        return {
        	type:"tuple_list",
        	elements:char,
        }
   }
   
   / LPAR head:(rhs_expression_property/logical_expression/range_notation/bit_selection_notation/tuple_notation) _
   tail:(COMMA (rhs_expression_property/logical_expression/range_notation/bit_selection_notation/tuple_notation) __)* RPAR by:tuple_by_notation?
   {
        if(by) {
        	var char = buildList(head, tail, 1);
            return {
            	type:"tuple_list",
                elements:char,
               	skip_by:by
          	}
       }
       var char = buildList(head, tail, 1);
       return {
        	type:"tuple_list",
        	elements:char
       }
   }
   / LPAR RPAR {
     return {
       type:"tuple_list",
       elements:null
     }
   }
   / bit_selection_notation
   // range_notation

range_notation
        = head:bit_selection_notation? ".." tail:(bit_selection_notation)? by:tuple_by_notation? {
		if(by){
        	return {
       			type:"range",
       			upper_bound:head,
       			lower_bound:tail,
                skip_by:by
     		}
        }
     	return {
       		type:"range",
       		upper_bound:head,
       		lower_bound:tail
     	}
   }
   // bit_selection_notation

bit_selection_notation
        = head:(tuple_dot_notation) tail:(LBRK LBRK property:(logical_expression/tuple_notation_no_bracket)? RBRK RBRK {return {property:property}})* {
     return tail.reduce(function(result, element) {
       return {
         type: "bit_selection",
         object: result,
         bit_sel: element.property,
       };
     }, head);

   }

/*
tuple_dot_notation
        = head:tuple_array_notation tail:(DOT property:(tuple_array_notation) {return {property:property}})* {
     return tail.reduce(function(result, element) {
       return {
         type: "tuple_object",
         object: result,
         property: element.property,
       };
     }, head);

   }
*/

tuple_dot_notation
	= head:tuple_array_notation tail:(DOT property:(tuple_array_notation) {return {property:property}})* {
 	return tail.reduce(function(result, element) {
     		return {
        		type: "tuple_object",
         		object: result,
         		property: element.property,
       		};
 		}, head);

	}
   

tuple_array_notation
        =  head:(lhs_var_name) tail:(LBRK property:(logical_expression/range_notation/bit_selection_notation/tuple_notation_no_bracket) 
        RBRK {return {property:property}})* {
   	return tail.reduce(function(result, element) {
       return {
         type: "tuple_array",
         object: result,
         index: element.property,
       };
     }, head);
   }

/*
tuple_array_notation
	= head:(lhs_var_name) tail:(property:(LBRK (logical_expression/range_notation/bit_selection_notation/tuple_notation_no_bracket) 
   	RBRK) {return property})* {
   		return tail.reduce(function(result, element) {
    		return prettyPrintArray(result,element)
  		},head);
	}
*/

   
lhs_expression
        = range_notation/tuple_notation

lhs_var_name
        = head:(identifier/constant) {return head}

rhs_expression_property
        = arg:identifier COLON head:(fcall_explicit/tuple_notation)?
   	{
    	if(head) {
        	return {
            	property:arg,
              	var:head
          	}
        }
       	return {
         property:arg,
         var:null
       	}
   	}
   /* arg:(x:identifier COLON {return x}) head:(fcall_explicit/tuple_dot_notation)? {
     return {
       property:arg,
       var:head
     }
   }*/

rhs_expression
        = head:(fcall_explicit/lhs_expression/scope_declaration) {return head}

constant
        = string_constant
   / numerical_constant

overload_notation
        = DOT DOT head:overload_name DOT DOT {
     var i, char=[]
     for (i=0; i<head.length; i++) {
       char.push(head[i][1]);
     }
     return {
       type:"overload_operator",
       value:head
     }
   }
   
overload_name
        = (!(overload_exception/line_terminator) .)+ {return text();}

overload_exception
        = line_terminator/white_space/"."/"#"/";"/","/"="/"("/")"/"["/"]"/"{"/"}"/"/"/"?"/"!"/"|"/"'"/'"'

identifier
        = !keyword bang:id_prefix? first:id_non_digit chars:id_char*
   {
     if(bang) {
     	//return bang+first+chars.join('')
       	return {
         	type:"identifier",
         	//prefix:bang,
         	value:bang+first+chars.join('')
       	}
     }
     //return first+chars.join('')
     return {
       type:"identifier",
       value:first+chars.join('')
     }
   }

id_non_digit
   = [a-z]
   / [A-Z]
   / [_]
   / [%]
   / [$]
   // [@]
   // [!]

id_char
   = [a-z]
   / [A-Z]
   / [0-9]
   / [_]

id_prefix
        = BANG
   / LOCAL_REGISTER
   // AMPERSAND

string_constant
        = '"' char:double_string* '"'  {
     //char = concatenate(char);
     return {type:"string_constant", value:char.join('')}
   }
   / "'" char:single_string* "'"  {
     //char = concatenate(char);
     return {type:"string_constant", value:char.join('')}
   }

double_string
        = !('"'/line_terminator) . {return text();}

single_string
        = !("'"/line_terminator) . {return text();}

numerical_constant
        = head:(boolean
   / hexa_decimal
   / binary
   / decimal_signed
   / decimal_digit) {return head;}

hexa_decimal
        = head:("0x") tail:(hex_digit)+ end:(("s"/"u") (decimal_digit ("bits"/"bit"))?)? {
     var i, char=[], char1=[];
     if (end){
        if (end[1]) {
        	return {
        		type:"numerical_constant",
                class:"hexa",
                value:head+tail.join(''),
               	sign:end[0],
                bits:end[1][0]
        	}
       }
       return {
       		type:"numerical_constant",
         	class:"hexa",
         	value:head+tail.join(''),
         	sign:end[0],
         	sign_value:null
       }
       //return char.join('');
  	}

     return {
       type:"numerical_constant",
       class:"hexa",
       value:head+tail.join('')
     }
     //return char.join('');
   }

binary
        = head:("0b") tail:binary_digit+ end:(("s"/"u") (decimal_digit ("bits"/"bit"))?)? {
     if (end) {
        if (end[1]) {
        return {
                type:"numerical_constant",
                class:"binary",
                value:tail[0].join(''),
                sign:end[0],
                bits:end[1][0]
                }
       }
        return {
        type:"numerical_constant",
           class:"binary",
        value:tail[0].join(''),
           sign:end[0],
           sign_value:null
        }
     }
     return {
       type:"numerical_constant",
       class:"binary",
       value:tail[0].join('')
     }
   }

boolean "true or false"
        = head:("true"
   / "TRUE"
   / "false"
   / "FALSE")
     {
       return {
         type:"boolean",
         value:head
       }
     }

decimal_signed
        = head:decimal_digit tail:(("s"/"u") (decimal_digit ("bits"/"bit"))?) {
   if (tail) {
        if (tail[1]) {
        return {
                type:"numerical_constant",
                class:"integer",
                value:head,
                sign:tail[0],
                bits:tail[1][0]
                }
       }
        return {
        type:"numerical_constant",
           class:"integer",
        value:head,
           sign:tail[0],
           sign_value:null
        }
     }

       return {
         type:"signed_integer",
         value:head,
         sign:tail[0],
         bits:tail[1][0]
       }
     }

decimal_digit "integer greater than or equal to zero"
        = head:("-")? tail:["?"0-9_]+ {
     if(head) {
     	//return head+tail.join('')
       return {
         type:"integer",
         value:head+tail.join(''),
         //sign:head
       }
     }
     //return tail.join('')
     return {
       type:"integer",
       value:tail.join(''),
     }
   }

non_zero_digit "integer greater than zero"
        = head:["?"1-9_]+ {return head.join('')}

binary_digit "binary digits"
        = ["?"0-1_]+

hex_digit "hexadecimal characters"
        = head:["?"A-Fa-f0-9_]+ {return head.join('')}

keyword "keywords"
        = (n:"if"
   / n:"else"
   / n:"elif"
   / n:"as"
   / n:"is"
   / n:"and"
   / n:"or"
   / n:"intersect"
   / n:"union"
   / n:"until"
   / n:"default"
   // n:"try"
   // n:"catch"
   / n:"C"
   / n:"I"
   / n:"N"
   / n:"in"
   / n:"for"
   / n:"while"
   / n:"end"
   / n:"by"
   / n:"return"
   / n:"when") !id_char

assignment_operator
  	= EQU
   / AS
   / T_PLUSEQU
   / STAREQU
   / PLUSEQU
   / MINUSEQU
   / LEFTEQU
   / RIGHTEQU

__
 	= x:(white_space
    / line_terminator_sequence
    // SEMI_COLON
    / comment)* {
    	var i, tmp=[]
        for(i=0;i<x.length;i++){
        	if(x[i].type=="comment"){
            	tmp.push(x[i])
            }
        }
        return tmp
    }

_
 	= (white_space
   / multi_line_comment_no_line_terminator)*

white_space
	= "\t"
 	/ "\v"
 	/ "\f"
 	/ " "
 	/ "\u00A0"
 	/ "\uFEFF"

EOS
	= head:(__ SEMI_COLON+
   	/ _ x:single_line_comment? line_terminator_sequence {return x}
   	/ __ EOF) 

EOF
        = !.

line_terminator
        = [\n\r\u2028\u2029]

line_terminator_sequence //end of line
        = "\n"
 / "\r\n"
 / "\r"
 / "\u2028"
 / "\u2029"

multi_line_comment_no_line_terminator
        = "###" (!("###"/ line_terminator) .)* "###"

comment "comment"
        = head:multi_line_comment            //multi-line comment
   / head:single_line_comment 

single_line_comment
        = "#" (!line_terminator .)* {//return "comment"
   		return {
        	type:"comment",
            start_pos:location().start.offset,
        	end_pos:location().end.offset,
            comment:text()
        }
   }

multi_line_comment
        = "###" (!"###" .)* "###" {//return "comment"
   		return {
        	type:"comment",
            start_pos:location().start.offset,
        	end_pos:location().end.offset,
            comment:text()
        }
   }

SEMI_COLON = ";" white_space*
LOCAL_REGISTER = x:"@"  {return x}
AMPERSAND       = x:"&" {return x}
FUNC_PIPE       = "|>" white_space*
BANG       =  x:"!"  ![=]   {return x}
LBRK       =  "["         white_space*
RBRK       =  white_space* x:"]"
LPAR       =  "("                 white_space*
RPAR       =  white_space* ")" _          
LBRACE       =  "{"               white_space*
RBRACE       =  white_space* "}"               
RANGE_DOT  = x:".."      {return x;}
STAR       =  x:"*"       {return {type:"arithmetic_operator",operation:x};}
PLUS       =  x:"+"      {return {type:"arithmetic_operator",operation:x};}
MINUS      =  x:"-"    {return {type:"arithmetic_operator",operation:x};}
TILDA      =  "~"
DIV        =  x:"/"  ![=]   {return {type:"arithmetic_operator",operation:x};}
LEFT       =  x:"<<"     {return {type:"shift_operator",operation:x};}
RIGHT      =  x:">>"     {return {type:"shift_operator",operation:x};}
T_PLUS     =  x:"++"                {return {type:"tuple_operator",operation:x};}
T_STAR     =  x:"**"                {return {type:"tuple_operator",operation:x};}
UNION      = x:"union"          {return {type:"tuple_operator",operation:x};}
INTERSECT      = x:"intersect"         {return {type:"tuple_operator",operation:x};}
LT         =  x:("<"/":<")  ![=]   {return {type:"relational_operator",operation:x};}
GT         =  x:(">"/":>")  ![=]    {return {type:"relational_operator",operation:x};}
LE         =  x:("<="/":<=")         {return {type:"relational_operator",operation:x};}
GE         =  x:(">="/":>=")         {return {type:"relational_operator",operation:x};}
EQUEQU     =  x:("=="/":==")         {return {type:"relational_operator",operation:x};}
ASSIGN_RIGHT    = x:"->"      {return x}
ISEQU      =  x:"is"               white_space* {return {type:"relational_operator",operation:x};}
BANGEQU    =  x:("!="/":!=")       white_space*    {return {type:"relational_operator",operation:x};}
EQU        =  x:("="/":=")  !"="   white_space*  {return x;}
AS                 = x:"as"                     white_space* {return x;}
HAT        =  x:"^"  ![=]        {return {type:"bitwise_operator",operation:x};}
OR         =  x:("or")           {return {type:"logical_operator",operation:x};}
AND        =  x:("and")          {return {type:"logical_operator",operation:x};}
IN         = x:"in"            {return {type:"in_operator",operation:x};} 
T_PLUSEQU  =  head:T_PLUS tail:EQU  {return head}
STAREQU    =  x:STAR EQU         {return x}
DIVEQU     =  "/="        white_space*
MODEQU     =  "%="        white_space*
PLUSEQU    =  x:PLUS EQU         {return x}
MINUSEQU   =  x:MINUS EQU      {return x}
LEFTEQU    =  x:LEFT EQU        {return x}
RIGHTEQU    =  x:RIGHT EQU       {return x}
COMMA      =  ","    __
COLON      =  ":"
DOUBLE_COLON       =  "::"                white_space*
DOT        =  x:"."          //{return x;}
BY                      = "by" white_space+
IF         = "if"         white_space+
ELIF         = "elif"         white_space+
ELSE         = "else"         white_space*
FOR         = "for"         white_space+
WHILE           = "while"               white_space+
WHEN         = "when"         white_space+
COMPILE_CHECK    = "C"         white_space+
ASSERTION        = "I"         white_space+
NEGATION         = "N"         white_space+
END                             = "end"         white_space*
RETURN                             = "return" _
SLASH                   = "/" white_space*
PIPE                    = "|"
